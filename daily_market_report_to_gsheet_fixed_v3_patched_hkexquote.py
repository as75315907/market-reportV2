...
def hk_hands_from_aastocks(code: str, timeout: int = 20):
    """Return 成交量(手) from AASTOCKS for a HK stock code like '03368' or '00825'.
    Returns int (hands) or None.
    """
    code = (code or "").strip()
    if not code:
        return None
    ...
    # 1) Robust HTML parse (preferred)
    try:
        soup = BeautifulSoup(html, "lxml")

        # AASTOCKS often shows a label cell then a value cell.
        # Try to locate the table cell containing '成交量' and read the next td.
        ...
        # Fallback: search in the visible text
        page_text = soup.get_text(" ", strip=True)
        for pat in [
            r"成交量\s*\(手\)\s*([0-9,]+)",
            r"成交量\s*([0-9,]+)\s*手",
    ...
    # 2) Last resort: regex on raw HTML
    for pat in [
        r"成交量\s*\(手\)\s*</[^>]+>\s*<[^>]+>\s*([0-9,]+)\s*<",
        r"成交量\s*</[^>]+>\s*<[^>]+>\s*([0-9,]+)\s*<",
    ]:
        m = re.search(pat, html)
        if m:
...
def main():
    ...
    for (r, code), tkr in zip(hk_rows, hk_tickers):
        ...
        # 港股：今日收盤 & 前一交易日收盤（以及開高低）都取到小數點後 3 位
        close = _round3(d.get("close"))
        prev  = _round3(d.get("prev_close"))
        opn   = _round3(d.get("open"))
    ...
    # Special handling ONLY for 03368 / 00825:
    # 1) Prefer AASTOCKS hands
    # 2) If AASTOCKS not available, try HKEX widget per-stock volume (shares) -> convert to hands by lot size
    # 3) If still not available, fallback to yfinance volume(share) -> convert to hands by lot size
    hands = hk_hands_from_aastocks(code)

    if str(os.environ.get("DEBUG_HKEX", "0")).strip() == "1":
        print(f"[HK HANDS] {code} AASTOCKS hands = {hands}")

    lot_size_map = {"03368": 500, "00825": 1000}

    if hands is None:
        try:
            vol_shares = hkex_get_volume_shares(code)
            lot = lot_size_map.get(code)
            if vol_shares is not None and lot:
                hands = int(vol_shares // lot)
                if str(os.environ.get("DEBUG_HKEX", "0")).strip() == "1":
                    print(f"[HK HANDS] {code} HKEX vol_shares={vol_shares} lot={lot} -> hands={hands}")
        except Exception as e:
            if str(os.environ.get("DEBUG_HKEX", "0")).strip() == "1":
                print(f"[HK HANDS] {code} HKEX fallback error: {e}")
    ...
    if str(os.environ.get("DEBUG_HKEX", "0")).strip() == "1":
        print(f"[HK HANDS] {code} final hands = {hands}")
